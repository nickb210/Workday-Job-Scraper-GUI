...
def job_search(job_title):
    ...
    #job_search_str = 'cyber security junior'
    #job_search_str = 'cyber security'
    job_search_str = job_title
    #ypeWriter("Searching for jobs containing '%s' ..." % job_search_str)
    ...
    #jobs = soup.find_all("div", {"class": "gwt-Label WBIP WKGP"})
    ...
    '''
    #locations = soup.find_all("span", {"class:", "gwt-InlineLabel WCAG WB5F"})
    ...
    for job_id in jobs_dict.keys():
        ...
        retStr += "%d. %s" % (count_jobs, job_title)
        retStr += "   JobID   : %s\n   Location: %s\n   Date    : %s\n   Link    : %s\n\n\n" % (job_id, job_location, job_post_date, link)
    ...
    #print(jobs_dict_json)
    
    #return jobs_dict_json
    #print(retStr)
    return retStr

#job_search("cyber security")